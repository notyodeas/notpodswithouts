...
//             as Iterable<dynamic>),
//         liberTxs = List<Transactio>.from(jsoschon['liberTxs']
//                 .map((x) => Transactio.fromJson(x as Map<String, dynamic>))
//             as Iterable<dynamic>),
//         fixumTxs = List<Transactio>.from(jsoschon['fixumTxs']
//                 .map((x) => Transactio.fromJson(x as Map<String, dynamic>))
//             as Iterable<dynamic>),
//         super.fromJson(jsoschon);
//   @override
...
// class TransactioP2PMessage extends P2PMessage {
//   Transactio tx;
//   TransactioP2PMessage(this.tx, String type, List<String> recieved)
//       : super(type, recieved);
//   TransactioP2PMessage.fromJson(Map<String, dynamic> jsoschon)
//       : tx = Transactio.fromJson(jsoschon['tx'] as Map<String, dynamic>),
//         super.fromJson(jsoschon);
//   @override
//   Map<String, dynamic> toJson() =>
...
//               json.decode(data) as Map<String, dynamic>);
//           propters.removeWhere((p) => rpp2pm.ids.any(
//               (identiatis) => identiatis == p.interioreRationem.identitatis));
...
//           TransactioP2PMessage tp2pm = TransactioP2PMessage.fromJson(
//               json.decode(data) as Map<String, dynamic>);
...
//                   );
//             }
//             // if (tp2pm.tx.isFurantur()) {
//             //   Print.nota(
//             //       nuntius: 'Nodi pecuniam furari conatur',
//             //       message: 'Node is trying to steal money');
//             // }
//             liberTxs.add(tp2pm.tx);
//           } else {
//             client.write(json.encode(RemoveTransactionsP2PMessage(
//                 List<String>.from([tp2pm.tx.interioreTransactio.identitatis]),
//                 'remove-liber-txs',
//                 List<String>.from(
//                     ['${client.address.address}:${client.port}'])).toJson()));
//           }
...
//                   json.decode(data) as Map<String, dynamic>);
//           liberTxs.removeWhere((l) =>
//               rtp2pm.ids.any((id) => id == l.interioreTransactio.identitatis));
//           for (String socket
...
//           Print.nota(
//               message:
//                   'recieved block ${op2pm.obstructionum.interioreObstructionum.obstructionumNumerus}',
//               nuntius:
//                   'accepit obstructionum ${op2pm.obstructionum.interioreObstructionum.obstructionumNumerus}');
...
//                 /// untill here we have the cleaner version of syncing
//                 for (Transactio tx in op2pm
...
//                   ;
//                   // now we hare hhere
//                   for (TransactioOutput output
//                       in tx.interioreTransactio.outputs) {
//                     if (!await Pera.isPublicaClavisDefended(
//                         output.publicaClavis, dir)) {
...
//                   //           .cashExs[tx.interioreTransactio.outputs.first
//                   //               .cashExIndex!]
//                   //           .interioreCashEx
//                   //           .signumCashEx
//                   //           .nof !=
//                   //       tx.interioreTransactio.outputs.first.app) {
...
//                 ///
//                 List<Transactio> transformOutputs = [];
//                 for (Transactio tx in op2pm
...
//                   }
//                 }
//                 List<Transactio> transformInputs = [];
//                 for (Transactio tx in op2pm
//                     .obstructionum.interioreObstructionum.liberTransactions) {
...
//                 for (List<TransactioInput> inputs in transformInputs
//                     .map((tx) => tx.interioreTransactio.inputs)) {
//                   for (TransactioInput input in inputs) {
...
//                         PublicKey.fromHex(
//                             Pera.curve(),
//                             tx.interioreTransactio.outputs[input.index]
//                                 .publicaClavis),
...
//                     } else {
//                       transformAble +=
//                           tx.interioreTransactio.outputs[input.index].pod;
//                     }
//                   }
//                 }
//                 BigInt transformed = BigInt.zero;

//                 for (List<TransactioOutput> outputs in transformOutputs
//                     .map((tx) => tx.interioreTransactio.outputs)) {
//                   for (TransactioOutput output in outputs) {
//                     transformed += output.pod;
//                   }
//                 }
...
//                     Generare.CONFUSSUS) {
//                   String gladiatorId = op2pm
//                       .obstructionum
//                       .interioreObstructionum
//                       .gladiator
//                       .input!
//                       .gladiatorIdentitatis;
//                   bool index = op2pm.obstructionum.interioreObstructionum
//                       .gladiator.input!.primis;
...
//                     return;
//                   }
//                   Telum turpiaDefensio = await Pera.turpiaGladiatoriaDefensione(
...
//                       (o) =>
//                           o.interioreObstructionum.gladiator.identitatis ==
//                           op2pm.obstructionum.interioreObstructionum.gladiator
//                               .input!.gladiatorIdentitatis);
...
//     if (propters.any((p) =>
//         p.interioreRationem.identitatis ==
//         propter.interioreRationem.identitatis)) {
//       propters.removeWhere((p) =>
//           p.interioreRationem.identitatis ==
//           propter.interioreRationem.identitatis);
//     }
...
//   void syncLiberTx(Transactio tx) async {
//     if (liberTxs.any((t) =>
//         t.interioreTransactio.identitatis ==
//         tx.interioreTransactio.identitatis)) {
//       liberTxs.removeWhere((t) =>
//           t.interioreTransactio.identitatis ==
//           tx.interioreTransactio.identitatis);
//     }
//     liberTxs.add(tx);
...
//           socket.split(':')[0], int.parse(socket.split(':')[1]));
//       soschock.write(
//           json.encode(TransactioP2PMessage(tx, 'liber-tx', sockets).toJson()));
//       soschock.listen((data) async {
//         RemoveTransactionsP2PMessage rtp2pm =
//             RemoveTransactionsP2PMessage.fromJson(
//                 json.decode(String.fromCharCodes(data).trim())
//                     as Map<String, dynamic>);
//         liberTxs.removeWhere((liber) =>
//             rtp2pm.ids.contains(liber.interioreTransactio.identitatis));
//       });
//     }
//   }

//   void syncExpressiTx(Transactio tx) async {
//     expressieTxs.add(tx);
//     for (String socket in sockets) {
//       Socket soschock = await Socket.connect(
//           socket.split(':')[0], int.parse(socket.split(':')[1]));
//       soschock.write(json
//           .encode(TransactioP2PMessage(tx, 'expressi-tx', sockets).toJson()));
//     }
//   }

//   void syncFixumTx(Transactio tx) async {
//     if (fixumTxs.any((t) =>
//         t.interioreTransactio.identitatis ==
//         tx.interioreTransactio.identitatis)) {
//       fixumTxs.removeWhere((t) =>
//           t.interioreTransactio.identitatis ==
//           tx.interioreTransactio.identitatis);
//     }
//     fixumTxs.add(tx);
...
//           socket.split(':')[0], int.parse(socket.split(':')[1]));
//       soschock.write(
//           json.encode(TransactioP2PMessage(tx, 'fixum-tx', sockets).toJson()));
//       soschock.listen((data) async {
//         RemoveTransactionsP2PMessage rtp2pm =
//             RemoveTransactionsP2PMessage.fromJson(
//                 json.decode(String.fromCharCodes(data).trim())
//                     as Map<String, dynamic>);
//         fixumTxs.removeWhere((fixum) =>
//             rtp2pm.ids.contains(fixum.interioreTransactio.identitatis));
//       });
//     }
//   }
...
//         (p) => ids.any((i) => i == p.interioreRationem.identitatis));
//     for (String socket in sockets) {
//       Socket soschock = await Socket.connect(
//           socket.split(':')[0], int.parse(socket.split(':')[1]));
...
//         (l) => ids.any((i) => i == l.interioreTransactio.identitatis));
//     for (String socket in sockets) {
//       Socket soschock = await Socket.connect(
//           socket.split(':')[0], int.parse(socket.split(':')[1]));
//       soschock.write(json.encode(
...
//     fixumTxs
//         .removeWhere((f) => ids.contains(f.interioreTransactio.identitatis));
//     for (String socket in sockets) {
//       Socket soschock = await Socket.connect(
//           socket.split(':')[0], int.parse(socket.split(':')[1]));
//       soschock.write(json.encode(
...
//     List<String> bases = args[1] as List<String>;
